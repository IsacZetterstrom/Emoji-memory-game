...
function removePlayerSelection() {
    //tar bort selection när man trycker på knappen "starta spelet"
    startSection.remove();
...
function getPlayerNames() {
    //tar emot spelarnas namn som är inskrivna
    if (
        ...
        // Sätter vi namnen i playerOne och playerTwo
...
function createNewCard(cardImg) {
    //funktion för att lägga till kort
...
function placeCards() {
    for (let a = 0; a < 2; a++) {
        // gör 2 gånger för att få ut 2 av varje kort
...
function shuffleClass() {
    ...
    let shuffledNumbers = classListIndex.sort(() => {
        //blandar alla nummer i "classListIndex"
...
function giveCardClass() {
    ...
    for (let i = 0; i < classListIndex.length; i++) {
        //lägger till ett klassnamn till "classListIndex"
...
function updateDisplays() {
    //funktion för att uppdatera allt i sidebar.
...
function flippedCard(card) {
    //ska spara och jämnföra / kolla om 2 är valda
    let main = document.querySelector('main'); // ta ut main i DOM
...
function isCardFlipped() {
    //kollar på alla kort om name är "flipped"
    let main = document.querySelector('main'); // ta ut main i DOM
    for (const article of main.children) {
...
function compareFlippedCard() {
    //jämnför korten om de är en match eller inte.
...
function customAlert(winner) {
    ...
    setTimeout(() => {
        // Timeout så att den körs när den läggs till annars gör den inget
...
//återställer alla variablar och kör om spelet
function playAgain() {
    flippedCardsList = [];
    classListIndex = [];
    gameTurn = 0;
...
function startGame() {
    //funktioner som kallas på när spelet startas.
